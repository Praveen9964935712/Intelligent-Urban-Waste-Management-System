  ...
  return (
    <AdminLayout>
      <main className="admin-module-placeholder">
        ...
        {loading ? <p>Loading complaints...</p> : !complaints.length ? <section className="admin-module-empty"><h2>No complaints found</h2></section> : <section className="admin-module-empty"><table><thead><tr><th>Complaint</th><th>Zone</th><th>Priority</th><th>Status</th><th>Actions</th></tr></thead><tbody>{complaints.map((complaint) => <tr key={complaint.id}><td><strong>#{complaint.id}</strong> {complaint.title}</td><td>{complaint.zone || "-"}</td><td>{complaint.priority || "-"}</td><td><select value={complaint.status || "PENDING"} onChange={(event) => changeStatus(complaint, event.target.value)}><option>PENDING</option><option>ASSIGNED</option><option>RESOLVED</option></select></td><td><button type="button" onClick={() => removeComplaint(complaint.id)}>Delete</button></td></tr>)}</tbody></table></section>}
      </main>
    </AdminLayout>
  );
}
