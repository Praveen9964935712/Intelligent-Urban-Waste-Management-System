    ...
    <div className="task-modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className="task-modal" role="dialog" aria-modal="true" aria-labelledby="task-modal-title">
        ...
        <div className="task-modal-body">
          <label>Complaint reference<input value={form.complaintId} onChange={(event) => update("complaintId", event.target.value)} disabled={isReadOnly || mode !== "create"} placeholder="Complaint ID" /></label>
          <label>Assigned staff<input value={form.staffId} onChange={(event) => update("staffId", event.target.value)} disabled={isReadOnly} placeholder="Staff ID" /></label>
        ...
        {!isReadOnly && <footer className="task-modal-footer"><button type="button" className="button-secondary" onClick={onClose}>Cancel</button><button type="button" className="button-primary" onClick={() => onSave({ ...form, complaintId: Number(form.complaintId), staffId: Number(form.staffId) })} disabled={saving}>{saving ? "Saving..." : "Save task"}</button></footer>}
      </section>
    </div>
  );
}
